...
def download_file_and_upload_to_s3(aws_s3_client, bucket_name, url, keep_local=False) -> str:
  ...
  with urlopen(url) as response:
    ...
    return "https://s3-{0}.amazonaws.com/{1}/{2}".format(
      getenv("aws_region_name"), bucket_name, file_name
    )


def upload_file(aws_s3_client, filename, bucket_name) -> bool:
...
def upload_file_obj(aws_s3_client, filename, bucket_name):
    with open(filename, "rb") as file:
        aws_s3_client.upload_fileobj(file, bucket_name, "filename.txt")
